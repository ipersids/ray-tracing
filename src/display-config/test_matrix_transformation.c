...
/*   Updated: 2025/05/20 15:20:41 by ipersids         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	test_matrix_transformation(void)
{
	...
	t_point res1 = matrix_multiply_vec3(m1, v1);
	...
	t_point res2 = matrix_multiply_vec3(inverse_m1, v1);
	...
	t_point res3 = matrix_multiply_vec3(half_quarter, v4);
	assert(equal(res3.x, 0.0) && equal(res3.y, sqrt(2) / 2.0) && equal(res3.z, sqrt(2) / 2.0));
	res3 = matrix_multiply_vec3(full_quarter, v4);
	...
	res3 = matrix_multiply_vec3(inverse_half_quarter, v4);
	assert(equal(res3.x, 0.0) && equal(res3.y, sqrt(2) / 2.0) && equal(res3.z, -(sqrt(2) / 2.0)));
	printf(PRINT_GREEN"success\n"PRINT_DEFAULT);
	...
	res3 = matrix_multiply_vec3(half_quarter, v5);
	assert(equal(res3.x, sqrt(2) / 2.0) && equal(res3.y, 0.0) && equal(res3.z, sqrt(2) / 2.0));
	res3 = matrix_multiply_vec3(full_quarter, v5);
	...
	res3 = matrix_multiply_vec3(half_quarter, v6);
	assert(equal(res3.x, -(sqrt(2) / 2.0)) && equal(res3.y, sqrt(2) / 2.0) && equal(res3.z, 0.0));
	res3 = matrix_multiply_vec3(full_quarter, v6);
	assert(equal(res3.x, -1.0) && equal(res3.y, 0.0) && equal(res3.z, 0.0));
	printf(PRINT_GREEN"success\n"PRINT_DEFAULT);
	...
	t_point res4 = matrix_multiply_vec3(m1, v7);
	assert(equal(res4.x, 5.0) && equal(res4.y, 3.0) && equal(res4.z, 4.0));
	arr[0] = 0.0;
	arr[1] = 1.0;
	m1 = matrix_shearing(arr);
	res4 = matrix_multiply_vec3(m1, v7);
	...
	t_point rot_p = matrix_multiply_vec3(rot_x, p);
	assert(equal(rot_p.x, 1.0) && equal(rot_p.y, -1.0) && equal(rot_p.z, 0.0));
	t_point scal_p = matrix_multiply_vec3(scal, rot_p);
	assert(equal(scal_p.x, 5.0) && equal(scal_p.y, -5.0) && equal(scal_p.z, 0.0));
	t_point transl_p = matrix_multiply_vec3(transl, scal_p);
	...
	t_point res5 = matrix_multiply_vec3(tramsf, p);
